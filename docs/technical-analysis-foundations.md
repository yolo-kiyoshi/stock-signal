...
TOMOSHIBIYORIでは、最初の期間平均を出した後、Wilder方式で前日の平均上昇幅・平均下落幅を
少しずつ更新します。単純に直近期間だけを毎日入れ替える方式より、一日前の境界で値が急に変わる
ことを抑えます。スイングでは14日、中長期の買い場では値動きを緩やかに見る28日を使います。
...
画面の抵抗帯候補は、人が都合よく線を引くことを減らすため、前後2営業日より高い局所高値を探し、
0.5 ATR以内に集まる高値をまとめます。同じ価格帯へ2回以上接触した場合だけ、最低0.2 ATRの幅を
持つ帯として最大3件を表示します。現在値がその帯を明確に上抜けた後は、抵抗候補から外します。
これは過去の上げ止まりを機械的に見つける方法の一つであり、市場参加者の注文を直接観測したもの
ではありません。必ず反落する壁でも、目標株価でもありません。
...
ダブルボトムは、下落してきた価格が2回安値を試してから上へ転じる「反転」の形です。本アプリは、
2安値が10営業日以上離れ、形が始まる前の値動きが下降だった場合だけダブルボトムと呼びます。
すでに上昇している途中の短いW字は、同じ見た目でも反転の前提が違うため購入候補へ使いません。
...
- 発生から3営業日以内は新規検討期間。
- 4〜20営業日は新規追随より保有管理を優先。
- 20営業日を超えたら古いシグナルとして期限切れ。
- 転換水準から1 ATRを超えて離れるか、想定上昇幅の半分以上を進んだら追いかけて買わない。
- 直近の価格が期待方向へ逆行すれば勢い弱化。
- 無効化水準を越えれば失敗。
- 形成値幅から計算した目標へ届けば目標到達。

たとえば上抜けから2日しか経っていなくても、すでに目標までの値幅を半分以上進んでいれば、
新規購入の余地は小さくなっています。この仕組みにより「昔ダブルボトムがあったから今も上昇」
「まだ3日以内だから今から買える」という誤った読み方を避けます。
...
### 9.1 海外市場から翌朝の日本株へ伝わる影響

日本市場が閉まった後も、米国株、為替、原油、米国金利は動きます。例えば原油が急騰すると、
輸送費や原材料費の上昇が警戒されます。金利が急上昇すると、将来利益の現在価値が小さくなるため、
特に成長株が売られやすくなります。その結果、個別銘柄のチャートが前日まで上向きでも、日本市場の
寄り付きで市場全体が下落する場合があります。

TOMOSHIBIYORIでは、これを個別銘柄のテクニカル方向とは別の「寄り付き前の外部環境」として
表示します。「強い警戒」は、その銘柄が必ず下落するという予測ではありません。同じ方向に複数の
外部指標が動いたため、新規購入を急がず、寄り付き後の値動きや株数を再確認するための注意表示です。

ニュースは原因を理解する助けになりますが、見出しだけで売買区分を決めません。初期版では、
米国株ETF、WTI原油、米10年債利回り、ドル円という数値の変化をルールで判定し、ニュースは
「なぜ数値が動いたか」を説明する補助材料とします。
...
形成安値を使う無効化水準は「パターン全体が壊れたか」を測る構造上の境界です。一方、新規購入で
その遠い水準まで損失を許すと、今の購入価格が良いかを判断しにくくなります。本アプリはスイング時、
転換水準の0.5 ATR下と構造的無効化水準の高い方を「実行用損切り目安」にします。

```text
実行用損切り目安 = max(構造的無効化水準, 転換水準 - 0.5 × ATR)
実行用R/R = (参考目標 - 現在値) / (現在値 - 実行用損切り目安)
```
...
実行用損切り目安 = max(1,790, 2,000 - 50) = 1,950円
リスク      = 2,020 - 1,950 = 70円
リワード    = 2,200 - 2,020 = 180円
実行用R/R   = 180 / 70 ≒ 2.57
```

構造的無効化水準1,790円はパターンの事後評価に残し、新規購入のサイズ計算には1,950円を使います。
どちらも終値基準の機械的な目安であり、1,950円で必ず売れる保証はありません。窓開けでは損失が
大きくなる可能性があります。
...
```text
1株あたりリスク = 70円
理論上限株数 = floor(50,000 / 70) = 714株
100株単位の参考上限 = 700株
必要資金の目安 = 2,020円 × 700株 = 1,414,000円
```
...
さらに画面の「今の買い準備」は、形と購入価格を分けます。

| 表示 | 初学者向けの意味 |
|---|---|
| 形を確認 | 上向きの材料はあるが、まだ買う条件ではない |
| 条件付き候補 | 支持・反発などはあるが、相対力や上値余地などが不足 |
| 購入条件が整う | 固定した確認条件をすべて満たした。注文前の追加確認へ進む段階 |
| 押し目待ち | 上昇しているが、短期間に進み過ぎて価格条件が悪い |
| 新規購入を避ける | 下降、失敗、データリスクなどにより新規購入を止める段階 |

例えば、急上昇した株はテクニカル方向が「上昇」でも、目標まで100円、損切りまで200円なら、
期待できる値幅より損失余地の方が大きい状態です。本アプリはこのケースを「押し目待ち」にします。
...
中長期モードでは「高くなり始めた銘柄を急いで追う」ことと、「上昇途中で一度下がった押し目を
待つ」ことを区別します。押し目とは、上昇トレンドの途中で起こる一時的な下落です。価格が安く
見えても、下降トレンドが続いているだけなら押し目ではありません。

本アプリは、まず20日線が60日線以上で、60日線も上向きかを確認します。そのうえで20日線、
60日線、直近20日安値、以前の抵抗帯を上抜けた後の価格帯を「支持候補」とします。支持候補へ
直近3営業日以内に日中で触れたこと、終値では帯の下へ崩れていないこと、前日終値を上回る陽線で
反発したこと、出来高または2日間の反発継続を別々に表示します。さらに直近20日のTOPIX相対力と、
次の抵抗帯までの上値余地を損切り幅で割ったR/Rを確認します。R/Rが1.5未満なら、形が整っても
「条件付き候補」に留めます。

例えば、100円付近を支持候補帯とします。日中安値が99円でも、終値が97円なら「触れて反発」では
なく「終値で割れた」可能性があります。反対に、安値99円、始値100円、終値103円で前日終値も
上回ったなら、支持候補を試した後の反発として扱えます。ただし、これだけで企業価値や将来利益は
分からないため、決算・財務・事業内容は別に確認します。

条件進捗が5分の4でも、80%の確率で上がるという意味ではありません。支持帯の研究では下落の
中断を捉える情報が報告される一方、単純な買い持ちを上回る収益までは確認できない研究もあります。
そのため本アプリは、支持帯への接触だけで購入候補にしません。
...
5. 購入イベントでは、期間中の最大上昇（MFE）と最大逆行（MAE）も確認する。
...
```text
実績ATR変化 = (実績日終値 - 判定翌営業日始値) ÷ 判定日時点ATR
```

判定日の終値を見てから同じ価格で買えるとは限らないため、本アプリは翌営業日の始値を仮の約定価格に
します。スイングでは±0.5 ATR、中長期では±1.0 ATRを境界にします。例えば翌日始値が1,000円、
...
同じ購入候補が5日続いても、5回別々に買うとは限りません。本アプリは連続する購入候補の初日だけを
一つの購入イベントとして数えます。MFEは期間中に最も有利だった高値、MAEは最も不利だった安値を
翌日始値からの率で表します。同じ日足の中で目標と損切りの双方へ触れた場合、どちらが先かは日足では
分からないため、保守的に損切りが先と扱います。

レポートは銘柄や期間を結果を見てから選べます。そのため、
...
また、手数料、税金、売買価格の滑りはこのレポートでは評価しません。MFE/MAEも日中の順序や
実際の約定可能性までは再現しません。
...
3. **チャートの大きな方向**：移動平均の並びと傾きを見る。必要なら指標を一つずつ非表示にする。
4. **勢いと上値**：RSIを過熱警告として読み、現在値から抵抗帯までの余地を見る。
5. **テクニカル方向**：価格が上昇・停滞・下落のどこにあるか確認する。
6. **投資検討区分**：購入候補、様子見、新規購入回避を確認する。
7. **チャートパターンの現在状態**：発生直後か、弱化・失敗・期限切れかを確認する。
8. **転換条件**：様子見なら、あと何が足りないか確認する。
9. **個別株固有の確認**：出来高、窓開け、流動性、決算、TOPIX相対力を見る。
10. **価格リスク**：現在値、無効化水準、参考目標、R/Rを見る。
11. **購入上限**：許容損失額から、過大な株数になっていないか確認する。
12. **AI最終確認**：明示的に実行し、最新情報、反対材料、引用元、未確認事項を確認する。
13. **企業内容**：引用先の決算資料、財務、事業内容を自分でも確認する。
