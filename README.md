...
投資検討区分の計算根拠と読み方は、初学者向けの
[投資判断ロジック入門](docs/investment-judgment-guide.md)を参照してください。
...
できます。マスタ同期前のコードだけを確認待ちにし、次回バッチで再照合します。通常の数字
...
初回投入後の`batch`は、最後に正常取り込みしたバルク対象日の翌日だけを差分取得します。
...
| TOPIX四本値 | 利用可能 | 20営業日トレンドを評価。同期待ちは明示 |
| 決算発表予定日 | 利用可能 | 分析期間内の決算を警戒条件として使用 |
...
# 差分取得、検証、保存、1・5・20営業日のルール分析を実行
docker compose run --rm batch
```

バッチは銘柄・取得元ごとの保存済み最終取引日を確認し、その翌日以降だけを要求します。
...
1日1回実行することを想定しています。通常は前回成功日の翌日以降だけを取得するため、
